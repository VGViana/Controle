# Compras Paraguai — Black Friday 2026

## 1. Resumo executivo

### Lista principal

| Prioridade | Produto            | Modelo                                     | Preço atual* |               Preço-alvo | Decisão                                      |
| ---------- | ------------------ | ------------------------------------------ | -----------: | -----------------------: | -------------------------------------------- |
| 1          | PS5                | Sony PS5 Slim Digital CFI-2115B 825 GB     |      US$ 629 |            **≤ US$ 500** | Comprar se atingir alvo                      |
| 2          | Mouse              | Keychron M6 Silent 1K                      |       US$ 59 |             **≤ US$ 50** | Comprar se atingir alvo                      |
| 3          | Hub                | UGREEN 10-in-1 15601                       |    US$ 39,50 |             **≤ US$ 35** | Comprar se atingir alvo                      |
| 4          | Luminária          | Xiaomi Mi LED Desk Lamp 1S MJTD01SYL 220 V |       US$ 28 |             **≤ US$ 33** | Comprar; excelente a ≤ US$ 30                |
| —          | SSD PS5 externo    | 1 TB                                       |            — |          **≤ US$ 60–65** | Somente se aparecer uma promoção excepcional |
| —          | SSD M.2 PS5        | 1–2 TB                                     |            — | **Ver seção específica** | Melhor expansão que SSD externo              |
| —          | Caixas de som      | Edifier MR3 ou equivalente                 |      ~US$ 79 |          **≤ US$ 65–70** | Opcional                                     |


\*Preços são referências de acompanhamento em 07/09/2026 e podem mudar.

**Regra:** o carrinho é um radar de preços. Não comprar simplesmente porque o produto está na lista.

---

# 2. Carrinho atual

1. [PS5 Digital Slim CFI-2115B 825 GB — Compras Paraguai](https://www.comprasparaguai.com.br/console-sony-playstation-5-digital-slim-cfi-2115b-4k-825-gb-branco-eua__5151472/)
2. [Keychron M6 Silent 1K — Compras Paraguai](https://www.comprasparaguai.com.br/mouse-sem-fio-keychron-m6-silent-1k-preto__5130973/)
3. [UGREEN 10-in-1 15601 — Compras Paraguai](https://www.comprasparaguai.com.br/hub-usb-c-10-in-1-ugreen-3usb-a-30hdmi4k30hzvgaethernetsdtf35mmusb-c-pd-15601__4411304/)
4. [Xiaomi Mi LED Desk Lamp 1S MJTD01SYL 220 V — Compras Paraguai](https://www.comprasparaguai.com.br/lampada-led-smart-xiaomi-mi-desk-lamp-1s-mjtd01syl-220v-branco__4454066/)

### Total atual

- PS5: US$ 629
- M6: US$ 59
- UGREEN: US$ 39,50
- Xiaomi 1S: US$ 28
- **Subtotal: US$ 755,50**

Se todos fossem comprados nesses preços e todos fossem considerados tributáveis:

- Cota terrestre: US$ 500
- Excedente: US$ 255,50
- Imposto: US$ 127,75
- **Custo total estimado: US$ 883,25**

Não usar esse valor como previsão definitiva: o câmbio usado pela Receita é o aplicável à declaração, e a classificação de cada bem deve seguir as regras da Receita.

---

# 3. Datas

## Black Friday Ciudad del Este 2026

**19 a 22 de novembro de 2026.**

Fonte oficial:
https://ccyscde.org.py/2026/07/24/black-friday-ciudad-del-este-2026-inicia-una-nueva-etapa-para-impulsar-el-comercio-el-turismo-y-el-desarrollo-regional/

### Melhor estratégia

#### Se houver apenas 1 dia de compras
**19/11/2026 — quinta-feira**

Ir cedo, priorizar os produtos de maior risco de acabar:

1. PS5
2. M6
3. UGREEN
4. Xiaomi

Motivo: é o primeiro dia oficial do evento e permite pegar estoque antes de outros compradores.

#### Se houver 2 dias
**19/11 + 20 ou 21/11**

- Dia 19: pesquisar, comparar e comprar as oportunidades realmente boas.
- Dia 20/21: aproveitar promoções que aparecerem e resolver itens que estavam sem estoque.
- Dia 22: tratar como último recurso.

**Não deixar o PS5 para o último dia.**

### Regra prática

Na semana anterior:

1. conferir estoque;
2. conferir SKU exato;
3. salvar os preços;
4. confirmar tensão elétrica da Xiaomi;
5. confirmar política de garantia;
6. conferir preço no Brasil.

---

# 4. Tributação brasileira

## Cota

Na entrada terrestre no Brasil:

**US$ 500 por viajante.**

A cota é:

- individual;
- intransferível;
- válida conforme a regra de intervalo de 30 dias;
- não pode ser simplesmente somada com a cota de outra pessoa.

Fonte oficial:
https://www.gov.br/receitafederal/pt-br/assuntos/aduana-e-comercio-exterior/viagens-internacionais/guia-do-viajante/entrada-no-brasil/cota-de-isencao-duty-free-e-bagagem-tributavel

## Imposto

Para bens tributáveis de bagagem que excedam a cota:

**50% sobre o valor excedente.**

Fórmula de planejamento:

```text
Imposto = (Valor total tributável - US$ 500) × 50%

Custo final = Valor total + Imposto
```

Exemplo:

```text
Compras = US$ 700

Excedente = 700 - 500 = US$ 200
Imposto = 200 × 50% = US$ 100

Total = US$ 800
```

Fonte:
https://www.gov.br/receitafederal/simples/pt-br/assuntos/aduana-e-comercio-exterior/viagens-internacionais/guia-do-viajante/entrada-no-brasil/bens-a-declarar-tributacao-e-calculo-do-imposto

### Regra fundamental

Depois de ultrapassar US$ 500:

> **Cada US$ 1 adicional de produto custa aproximadamente US$ 1,50 no total**, considerando somente o imposto de 50% sobre o excedente.

Por isso não adicionar produtos baratos simplesmente porque "já estou pagando imposto".

---

# 5. Pessoal / usado / isenção

Não presumir que um eletrônico novo comprado no Paraguai será automaticamente isento como "uso pessoal".

A Receita estabelece condições específicas para bens de uso pessoal, incluindo uso próprio, necessidade relacionada à viagem, condição de usado e compatibilidade de natureza/quantidade.

Para este planejamento, usar uma abordagem conservadora:

> **considerar os eletrônicos novos como tributáveis.**

Isso evita montar um orçamento baseado em uma interpretação otimista.

Fonte:
https://www.gov.br/receitafederal/pt-br/assuntos/aduana-e-comercio-exterior/viagens-internacionais/guia-do-viajante/entrada-no-brasil/cota-de-isencao-duty-free-e-bagagem-tributavel

---

# 6. Estratégia de preço

## PS5

Produto:
Sony PS5 Slim Digital CFI-2115B 825 GB.

Preço atual observado no Compras Paraguai:
~US$ 615–629 dependendo da oferta.

Link:
https://www.comprasparaguai.com.br/sony-playstation-5-slim-825gb-digital-edition_66229/

### Faixas

| Preço | Decisão |
|---:|---|
| **≤ US$ 430** | Excelente; comprar |
| **US$ 430–470** | Excelente |
| **US$ 470–500** | Muito bom |
| **US$ 500–550** | Comparar fortemente com Brasil |
| **US$ 550–570** | Só se o Brasil estiver ruim |
| **> US$ 570** | Não comprar no Paraguai |

### Motivo

O PS5 é o item que mais pesa no orçamento e também o item mais difícil de substituir por outro produto.

**Prioridade máxima.**

Não comprar uma versão diferente só porque está barata.

Conferir:

- CFI-2115B;
- Digital;
- 825 GB;
- produto novo;
- lacrado;
- nota fiscal;
- garantia;
- região/modelo;
- conteúdo da caixa.

---

# 7. Keychron M6 Silent 1K

Link:
https://www.comprasparaguai.com.br/mouse-sem-fio-keychron-m6-silent-1k-preto__5130973/

Preço atual observado:
**US$ 59.**

### Faixas

| Preço | Decisão |
|---:|---|
| **≤ US$ 45** | Excelente |
| **US$ 45–50** | Comprar |
| **US$ 50–55** | Aceitável |
| **US$ 55–59** | Esperar |
| **> US$ 59** | Não comprar |

### Por que comprar

É uma compra de ergonomia/produtividade:

- formato ergonômico;
- ~78 g;
- sensor PAW3395;
- 2.4 GHz;
- Bluetooth;
- USB-C;
- roda lateral;
- 1000 Hz;
- cliques silenciosos.

**Não trocar pelo MX Master 3S automaticamente.**

Só considerar MX Master 3S se aparecer por aproximadamente **US$ 50 ou menos**.

---

# 8. UGREEN 10-in-1 15601

Link:
https://www.comprasparaguai.com.br/hub-usb-c-10-in-1-ugreen-3usb-a-30hdmi4k30hzvgaethernetsdtf35mmusb-c-pd-15601__4411304/

Preço de referência:
**US$ 39,50.**

### Faixas

| Preço | Decisão |
|---:|---|
| **≤ US$ 30** | Excelente |
| **US$ 30–35** | Comprar |
| **US$ 35–40** | Bom, mas observar |
| **US$ 40–45** | Só se precisar muito |
| **> US$ 45** | Não comprar |

### ATENÇÃO — IdeaPad 3 15ALC6

O USB-C do IdeaPad 3 15ALC6 é **USB-C 3.2 Gen 1 para dados**.

Ele não fornece:

- DisplayPort Alt Mode;
- USB-C Power Delivery para carregar o notebook.

Portanto:

```text
USB-C do Lenovo
        ↓
UGREEN
        ├── USB-A → FUNCIONA
        ├── Ethernet → FUNCIONA
        ├── SD/TF → FUNCIONA
        ├── áudio → FUNCIONA
        ├── HDMI → NÃO usar esperando vídeo pelo USB-C
        └── PD → NÃO usar esperando carregar o Lenovo
```

O notebook continua usando o HDMI próprio para vídeo.

Fonte Lenovo:
https://psref.lenovo.com/syspool/Sys/PDF/IdeaPad/IdeaPad_3_15ALC6/IdeaPad_3_15ALC6_Spec.html

### Conclusão

Ainda vale a pena como hub de conectividade.

Mas **não compre esperando uma dock completa para o notebook**.

---

# 9. Xiaomi Mi LED Desk Lamp 1S

Link:
https://www.comprasparaguai.com.br/lampada-led-smart-xiaomi-mi-desk-lamp-1s-mjtd01syl-220v-branco__4454066/

Preço observado:
**US$ 28.**

### Faixas

| Preço | Decisão |
|---:|---|
| **≤ US$ 28** | Excelente |
| **US$ 28–33** | Comprar |
| **US$ 33–36** | Só se necessário |
| **> US$ 36** | Esperar |

### Por que combina com o setup

- 2600–5000 K;
- 520 lm;
- CRI/Ra 90;
- brilho regulável;
- Wi-Fi;
- controle por aplicativo;
- Alexa;
- Google Assistant;
- HomeKit;
- formato próprio para iluminar mesa;
- dobrável.

A faixa quente permite usar aproximadamente:

**2700–3000 K** → uso noturno/ambiente quente.

### ATENÇÃO: 220 V

O produto escolhido é explicitamente **220 V**.

Antes de pagar:

> **CONFIRMAR NA CAIXA/ETIQUETA QUE É A VERSÃO 220–240 V.**

E confirmar que a tomada onde ela será usada é compatível.

Não comprar a versão errada.

---

# 10. SSD externo para PS5

## Não é prioridade.

Um SSD externo USB pode ser usado no PS5 como armazenamento estendido.

Porém:

> **Jogos de PS5 não rodam diretamente do SSD externo.**

Você pode armazenar/mover os jogos para ele, mas precisa copiá-los de volta para o armazenamento interno ou para um SSD M.2 interno compatível quando quiser jogar.

Jogos PS4 podem ser executados diretamente do armazenamento USB.

Fonte oficial:
https://www.playstation.com/pt-br/support/hardware/ps5-extended-storage/

### Se aparecer uma oportunidade

Comprar somente se:

- 1 TB;
- USB 3.x SuperSpeed;
- marca confiável;
- ≤ **US$ 60–65**;
- garantia verificável.

Preferir:

- Samsung;
- SanDisk;
- Crucial;
- Kingston;
- WD.

Evitar:

- SSD "1 TB" genérico;
- marcas desconhecidas;
- capacidade suspeita;
- preços absurdamente baixos;
- produto sem nota/garantia.

### Minha decisão

**Não colocar no carrinho agora.**

Se o preço do SSD externo não for excepcional, guardar o dinheiro para um **SSD M.2 interno do PS5**.

---

# 11. SSD M.2 para PS5 — melhor expansão

Se o armazenamento do PS5 realmente ficar pequeno, esta é a expansão que interessa.

O PS5 aceita SSD M.2 compatível para:

- baixar jogos;
- instalar jogos;
- executar jogos diretamente.

Fonte oficial:
https://www.playstation.com/pt-br/support/hardware/ps5-install-m2-ssd/

### Alvo

#### 1 TB

Comprar se encontrar um SSD de marca confiável por:

**≤ US$ 70–80**

#### 2 TB

**Melhor escolha de longo prazo.**

Comprar se aparecer:

**≤ US$ 120–140**

### Especificações

Não comprar somente pelo nome "NVMe".

Conferir:

- PCIe NVMe;
- formato compatível;
- velocidade de leitura dentro da especificação do PS5;
- dissipador;
- dimensões;
- marca;
- garantia.

### Preferência

1. 2 TB bom e barato
2. 1 TB muito barato
3. 4 TB somente se aparecer uma promoção excepcional

**Não comprar SSD M.2 agora apenas porque o PS5 tem 825 GB.**

Primeiro usar o console.

---

# 12. SATA 2,5" para o IdeaPad 3 15ALC6

O PSREF do IdeaPad 3 15ALC6 indica suporte a:

- 1× M.2 SSD;
- 1× unidade 2,5";
- nos modelos com bateria de 38 Wh, até dois drives;
- unidade 2,5" de até 1 TB.

Fonte:
https://psref.lenovo.com/syspool/sys/pdf/ideapad/ideapad_3_15alc6/ideapad_3_15alc6_spec.pdf

### Mas existe uma pegadinha

Não basta comprar um SSD SATA.

Dependendo da configuração física, pode ser necessário:

- cabo SATA;
- suporte/caddy;
- parafusos;
- espaço físico;
- verificar a bateria e a montagem interna.

### Situação atual

Você usa ~100 GB do SSD de 512 GB.

Portanto:

> **NÃO COMPRAR SATA AGORA.**

Só comprar quando houver necessidade real.

### Preço-alvo futuro

Para 1 TB SATA de marca confiável:

**≤ US$ 55–65** = interessante.

Se estiver:

**US$ 80+** = comparar com NVMe externo/SSD M.2 e com preços no Brasil.

---

# 13. SSD M.2 para o notebook

### NÃO COMPRAR AGORA.

Você já tem:

**512 GB**

e utiliza uma pequena parcela.

Não faz sentido transformar dinheiro + imposto em capacidade que você não precisa.

### Quando comprar

Somente se:

- uso ultrapassar ~350–400 GB;
- começar a faltar espaço;
- precisar trabalhar com arquivos grandes;
- precisar de segundo sistema;
- quiser separar dados/sistema.

### Futuramente

Preferir:

- 1 TB ou 2 TB;
- NVMe;
- marca confiável.

Não pagar prêmio por PCIe 4.0 se o uso real não justificar.

---

# 14. RAM para o notebook

## NÃO COMPRAR.

Esta é uma das decisões mais importantes deste documento.

Seu IdeaPad 3 15ALC6 específico possui:

- **4 GB soldados**
- **8 GB SO-DIMM**
- total atual: **12 GB**

O PSREF da Lenovo informa que a configuração com 4 GB soldados + 8 GB SO-DIMM chega a **12 GB**.

Existe uma configuração de placa com 8 GB soldados + 8 GB SO-DIMM que chega a 16 GB, mas isso **não significa que seu notebook possa simplesmente trocar o módulo e chegar a 20 GB**.

Fonte:
https://psref.lenovo.com/syspool/Sys/PDF/IdeaPad/IdeaPad_3_15ALC6/IdeaPad_3_15ALC6_Spec.html

### Portanto

**Não comprar 16 GB SO-DIMM para tentar fazer 20 GB.**

No seu aparelho atual:

> 4 GB soldados + 8 GB removíveis = 12 GB.

E seu uso atual está adequado.

---

# 15. MacBook para Linux

## NÃO COMPRAR para esta viagem.

MacBook pode rodar Linux, mas não é a melhor escolha para o seu objetivo.

### Apple Silicon

M1/M2/M3 podem rodar Linux através do projeto Asahi Linux/Fedora Asahi Remix.

O suporte está muito bom nos M1/M2, e o suporte a M3 foi oficialmente incorporado ao instalador em setembro de 2026.

Fontes:
https://asahilinux.org/about/
https://asahilinux.org/fedora/
https://asahilinux.org/2026/09/m2-episode-1/

### Porém

Apple Silicon é ARM.

Isso significa:

- algumas aplicações precisam de versões ARM;
- alguns jogos podem ser problemáticos;
- compatibilidade de software é diferente de x86;
- não é tão simples quanto instalar Arch/Fedora em um ThinkPad AMD.

### M3

O suporte Linux ao M3 está avançando muito, mas em setembro de 2026 ainda há limitações relevantes, incluindo GPU/3D e alguns recursos de firmware.

### Intel Mac

Intel Mac pode rodar Linux, inclusive modelos com T2 usando t2linux, mas eu não compraria um notebook Intel antigo em 2026 somente para Linux.

### Decisão

Se a prioridade for:

> **Linux + manutenção + compatibilidade + custo-benefício**

preferir:

**ThinkPad AMD > MacBook**

---

# 16. ThinkPad AMD Gen 2/Gen 3

Essa é a única alternativa de notebook que eu manteria no radar.

### Modelos interessantes

Prioridade:

1. **ThinkPad T14 Gen 2 AMD**
2. **ThinkPad T14 Gen 3 AMD**
3. **ThinkPad E14 Gen 2 AMD**
4. **ThinkPad E14 Gen 3 AMD**

### CPU

Preferir:

- Ryzen 5;
- Ryzen 7;
- 16 GB RAM;
- SSD 512 GB ou maior.

### T14 vs E14

**T14:**
- construção superior;
- melhor linha profissional;
- geralmente melhor teclado;
- melhor manutenção;
- melhor compra usada/seminova.

**E14:**
- mais barato;
- ainda muito competente;
- melhor custo-benefício se a diferença for grande.

### Preços

No Paraguai, atualmente há ThinkPad E16 Gen 2 AMD por cerca de **US$ 830**, mostrando que notebooks corporativos novos não necessariamente serão baratos.

Fonte:
https://www.comprasparaguai.com.br/busca/?q=thinkpad

### Regra

Não comprar um ThinkPad simplesmente porque é ThinkPad.

Só considerar se:

- CPU AMD;
- 16 GB;
- SSD ≥ 512 GB;
- tela decente;
- estado excelente;
- preço muito abaixo de um notebook novo equivalente.

### Para esta viagem

**Não comprar.**

Seu IdeaPad atual continua sendo suficiente.

---

# 17. Caixas de som para o PC

Você pediu:

> boa relação custo-benefício + P2/3,5 mm.

## Melhor alvo: Edifier MR3

Preço atual observado:
**~US$ 79**.

Características:

- 36 W RMS;
- 18 W + 18 W;
- Bluetooth 5.4;
- TRS;
- RCA;
- AUX;
- woofer 3,5";
- tweeter de seda 1";
- 52 Hz–40 kHz;
- saída de fone 3,5 mm.

Fonte:
https://www.comprasparaguai.com.br/caixa-de-som-edifier-mr3-20-ativa-36-w-rms-bluetooth-branco__5463395/

### Preço-alvo

| Preço | Decisão |
|---:|---|
| **≤ US$ 60** | Excelente |
| **US$ 60–70** | Comprar |
| **US$ 70–80** | Bom |
| **> US$ 80** | Esperar |

### Alternativas

**Edifier G2000 Pro 2.0**
- ~US$ 109;
- 32 W RMS;
- AUX;
- USB-C;
- Bluetooth;
- RGB.

Só vale se você realmente quiser o formato gamer/RGB.

**Edifier R1280DBS**
- ~US$ 119;
- 42 W RMS;
- Bluetooth;
- RCA/óptico/coaxial;
- melhor para som mais encorpado e uso doméstico.

### Minha decisão

Para seu setup:

> **MR3 ≤ US$ 70 = adicionar à lista.**

Mas **não colocar no carrinho agora**.

Seu carrinho já tem bastante valor tributável.

---

# 18. Lojas

## Prioridade

### 1. Nissei

Boa primeira opção para:

- Xiaomi;
- UGREEN;
- Keychron;
- periféricos;
- eletrônicos;
- produtos com SKU claro.

Site:
https://nissei.com/

### 2. Shopping China

Boa para:

- eletrônicos;
- consoles;
- Apple;
- notebooks;
- áudio;
- grandes marcas.

Site:
https://www.shoppingchina.com.py/

### 3. Cellshop

Boa para:

- eletrônicos;
- informática;
- consoles;
- notebooks;
- periféricos.

Site:
https://cellshop.com.py/

### 4. Compras Paraguai

**Não é loja.**

É comparador.

Usar para:

1. descobrir preço;
2. descobrir lojas;
3. acompanhar histórico;
4. criar alertas;
5. comparar ofertas.

Site:
https://www.comprasparaguai.com.br/

### Regra

Para itens caros:

> **Comparar pelo menos 2 lojas antes de pagar.**

---

# 19. Como comprar nas lojas

## Antes de sair do hotel

Conferir no celular:

- modelo;
- SKU;
- preço;
- estoque;
- loja;
- horário;
- garantia.

## Dentro da loja

Não aceitar:

> "É praticamente o mesmo."

Conferir:

- modelo exato;
- capacidade;
- tensão;
- cor;
- geração;
- código;
- estado;
- acessórios.

### PS5

Conferir especialmente:

```text
CFI-2115B
Digital
825 GB
Novo
Lacrado
```

### Xiaomi

Conferir:

```text
MJTD01SYL
220–240 V
```

### M6

Conferir:

```text
M6 Silent 1K
Preto
```

### UGREEN

Conferir:

```text
15601
10-in-1
```

---

# 20. Situações boas

## Cenário A — excelente

```text
PS5       ≤ US$ 470
M6        ≤ US$ 45
UGREEN    ≤ US$ 30
Xiaomi    ≤ US$ 30
```

Comprar os quatro.

Não adicionar mais nada.

## Cenário B — bom

```text
PS5       US$ 470–500
M6        US$ 45–50
UGREEN    US$ 30–35
Xiaomi    US$ 30–33
```

Comprar os quatro se não houver alternativa brasileira melhor.

## Cenário C — PS5 barato, acessórios caros

Exemplo:

```text
PS5       US$ 450
M6        US$ 59
UGREEN    US$ 45
Xiaomi    US$ 36
```

Comprar:

- PS5;
- Xiaomi se realmente quiser;
- M6/UGREEN somente se a diferença para o Brasil compensar.

## Cenário D — PS5 caro

```text
PS5 > US$ 570
```

**Não comprar por impulso.**

Pesquisar Brasil.

---

# 21. Situações ruins

## PS5 acima de US$ 600

Não comprar.

O preço atual não é suficientemente agressivo.

## M6 acima de US$ 55

Esperar.

## Hub acima de US$ 45

Comprar no Brasil ou escolher um hub menor.

## Xiaomi acima de US$ 36

Esperar.

## Produto sem nota

**Não comprar.**

## Produto sem garantia clara

**Não comprar item caro.**

## Produto "original, mas sem caixa"

Para itens caros:

**Não comprar.**

## Loja tentando substituir o modelo

Parar e conferir novamente.

---

# 22. Situação de contingência: produto sem estoque

Não substituir imediatamente.

Fazer:

1. procurar Nissei;
2. procurar Shopping China;
3. procurar Cellshop;
4. procurar no Compras Paraguai;
5. comparar o SKU;
6. comparar o preço;
7. decidir.

### Não fazer

Comprar uma versão inferior só porque estava na lista.

---

# 23. Situação de contingência: preço mudou na loja

O preço mostrado no Compras Paraguai é referência.

Pode mudar.

Se a diferença for:

**≤ US$ 5:** aceitável se ainda estiver dentro do preço-alvo.

**US$ 5–15:** comparar outra loja.

**> US$ 15:** parar e pesquisar novamente.

---

# 24. Situação de contingência: loja oferece desconto em dinheiro

Não assumir que qualquer desconto é vantajoso.

Calcular:

```text
Preço real = preço pago + eventual custo financeiro + risco de garantia
```

E exigir nota fiscal/recibo.

---

# 25. Situação de contingência: ultrapassei o orçamento

Ordem de corte:

1. acessórios;
2. caixa de som;
3. M6;
4. UGREEN;
5. Xiaomi;
6. **PS5 por último.**

Mas se o PS5 estiver caro demais, o PS5 também deve ser cancelado.

**Não comprar algo caro só porque foi a viagem inteira para isso.**

---

# 26. Cálculos rápidos do carrinho

## Carrinho atual

```text
PS5       629
M6         59
UGREEN   39,50
Xiaomi     28
----------------
Total    755,50

Imposto:
(755,50 - 500) × 0,50
= 127,75

Total:
883,25
```

## Carrinho bom

```text
PS5       500
M6         50
UGREEN     35
Xiaomi     33
----------------
Total    618

Imposto:
(618 - 500) × 0,50
= 59

Total:
677
```

## Carrinho excelente

```text
PS5       470
M6         45
UGREEN     30
Xiaomi     28
----------------
Total    573

Imposto:
(573 - 500) × 0,50
= 36,50

Total:
609,50
```

## Carrinho ideal

```text
PS5       450
M6         45
UGREEN     30
Xiaomi     28
----------------
Total    553

Imposto:
(553 - 500) × 0,50
= 26,50

Total:
579,50
```

Esses cálculos são **planejamento**, não substituem a e-DBV.

---

# 27. O que realmente comprar

## COMPRAR SE O PREÇO FOR BOM

- [ ] PS5 Slim Digital CFI-2115B
- [ ] Keychron M6 Silent 1K
- [ ] UGREEN 15601
- [ ] Xiaomi Mi LED Desk Lamp 1S

## OBSERVAR, MAS NÃO COLOCAR NO CARRINHO

- [ ] SSD M.2 PS5 1 TB
- [ ] SSD M.2 PS5 2 TB
- [ ] SSD externo PS5 1 TB
- [ ] Edifier MR3

## NÃO COMPRAR AGORA

- [ ] RAM do IdeaPad
- [ ] SSD do notebook
- [ ] SATA 2,5" do notebook
- [ ] MacBook
- [ ] ThinkPad
- [ ] outro teclado
- [ ] outro monitor
- [ ] braço de monitor
- [ ] suporte de notebook
- [ ] decoração

---

# 28. Ordem de prioridade no dia da compra

```text
1. PS5
2. Xiaomi 1S
3. Keychron M6
4. UGREEN 15601
5. SSD M.2 PS5 — somente promoção excepcional
6. Edifier MR3 — somente promoção excepcional
```

Se o orçamento apertar:

```text
PS5
↓
M6
↓
UGREEN
↓
Xiaomi
↓
todo o resto
```

---

# 29. Regra final

Antes de comprar qualquer produto novo no Paraguai, responder:

```text
1. Eu realmente preciso disso?
2. Existe no Brasil por preço parecido?
3. O preço paraguaio continua bom depois do imposto?
4. A loja é confiável?
5. O modelo/SKU é exatamente o correto?
6. Tenho nota?
7. Tenho garantia?
8. Isso vale ocupar parte da minha cota?
```

Se qualquer resposta importante for "não":

> **NÃO COMPRAR.**

---

# 30. Estado final desejado

O melhor resultado da viagem não é voltar com muitas sacolas.

É voltar com:

- PS5 muito mais barato que no Brasil;
- mouse excelente;
- hub útil;
- iluminação inteligente;
- nenhum produto inútil;
- nenhum upgrade desnecessário;
- orçamento controlado;
- documentação/garantia dos produtos;
- imposto corretamente considerado.

**Objetivo: poucas compras, todas boas.**

---

## Fontes principais

- Receita Federal — cota e isenções:
  https://www.gov.br/receitafederal/pt-br/assuntos/aduana-e-comercio-exterior/viagens-internacionais/guia-do-viajante/entrada-no-brasil/cota-de-isencao-duty-free-e-bagagem-tributavel
- Receita Federal — cálculo do imposto:
  https://www.gov.br/receitafederal/simples/pt-br/assuntos/aduana-e-comercio-exterior/viagens-internacionais/guia-do-viajante/entrada-no-brasil/bens-a-declarar-tributacao-e-calculo-do-imposto
- Black Friday CDE 2026:
  https://ccyscde.org.py/2026/07/24/black-friday-ciudad-del-este-2026-inicia-una-nueva-etapa-para-impulsar-el-comercio-el-turismo-y-el-desarrollo-regional/
- Lenovo IdeaPad 3 15ALC6 PSREF:
  https://psref.lenovo.com/syspool/Sys/PDF/IdeaPad/IdeaPad_3_15ALC6/IdeaPad_3_15ALC6_Spec.html
- PlayStation — armazenamento USB:
  https://www.playstation.com/pt-br/support/hardware/ps5-extended-storage/
- PlayStation — SSD M.2:
  https://www.playstation.com/pt-br/support/hardware/ps5-install-m2-ssd/
- Asahi Linux:
  https://asahilinux.org/
- Fedora Asahi Remix:
  https://asahilinux.org/fedora/
- Compras Paraguai:
  https://www.comprasparaguai.com.br/
