---
id: Atos Administrativos
aliases:
  - Ato Administrativo
---
# **Atos Administrativos:**

- Manifestar a <span style="color:#d3869b">**vontade do Estado**</span>;
- Espécie de <span style="color:#d3869b">**ato jurídico**</span>;
- Depende da vontade;
- Produção de <span style="color:#d3869b">**efeitos jurídicos**</span> com **<span style="color:#d3869b">fim público</span>**;
- Predomínio do <span style="color:#d3869b">**direito público**</span>;
- Manifestação/declaração <span style="color:#d3869b">**unilateral**</span>;
- <span style="color:#d3869b">**Controle**</span> do [[13. Poder Judiciário]].  
# **1. Atos da administração:**

- Mais amplo;
    - Diversas espécies:
        - <span style="color:#d3869b">**atos administrativos**</span>;
        - atos do direito privado;
        - atos materiais;
        - atos de conhecimento, opinião, juízo ou valor;
        - atos políticos;
        - contratos, convênios administrativos;
        - atos normativos.  
              
            

# **2. Fatos administrativos:** 

- Três sentidos:
	- <span style="color:#83a598">**atividade material**</span> decorrente de um **ato administrativo**;
	- <span style="color:#83a598">**atuação administrativa**</span> que produz **efeitos** **jurídicos indiretamente**;
	- <span style="color:#83a598">**evento da natureza**</span> que produz **efeitos jurídicos**.
	    - <span style="color:#fb4934">**Não possuem**</span> como <span style="color:#fb4934">**finalidade**</span> a produção de <span style="color:#83a598">**efeitos jurídicos**</span> (conquanto, eventualmente,   
	        possam decorrer efeitos jurídicos deles);
	    - <span style="color:#fb4934">**Não há manifestação ou declaração de vontade**</span>, com conteúdo jurídico, da administração   
	        pública;
	    - <span style="color:#fb4934">**Não**</span> faz sentido falar em “<span style="color:#fb4934">**presunção de legitimidade**</span>” de fatos administrativos;
	    - <span style="color:#fb4934">**Não existe revogação ou anulação**</span> de fatos administrativos;
	    - <span style="color:#fb4934">**Não**</span> faz sentido falar em <span style="color:#fb4934">**fatos administrativos discricionários e vinculados.</span>**

# **3. Silêncio administrativo:**

- <span style="color:#83a598">**Omissão da administração** </span>quando há o <span style="color:#83a598">**dever de se pronunciar**</span>;
- Dependem de <span style="color:#83a598">**previsão legal**</span>;
    - a lei prescreve que o <span style="color:#83a598">**silêncio significa manifestação positiva**</span> (anuência tácita);
    -  a lei dispõe que a <span style="color:#83a598">**omissão significa manifestação denegatória**</span> (pedido negado).
- <span style="color:#83a598">**Omissão da lei quanto aos resultados do silêncio:**</span>
    - <u>_após o decurso do prazo_</u> o juiz:
        - **ato vinculado:**  <span style="color:#83a598">**defere o pedido**</span> ou <span style="color:#83a598">**manda**</span> a administração <span style="color:#83a598">**deferir**</span>;
        - **ato discricionário**: <span style="color:#83a598">**prazo para a manifestação**</span> da administração.

# **4. Atributos:**

- <span style="color:#83a598">**Presunção de legitimidade ou veracidade</span> (presente em todos os atos administrativos);**
- <span style="color:#d3869b"> **Imperatividade</span>;**
- <span style="color:#fb4934">**Autoexecutoriedade</span>;**
- <span style="color:#83a598">**Tipicidade</span> (presente em todos os atos administrativos).**

## **4.1 Presunção de legitimidade ou veracidade:**

- **Legitimidade:** ato em conformidade com a lei;
- **Veracidade:** fatos verdadeiros;
- **Fundamentos:**
    - Necessidade de <span style="color:#83a598">**celeridade**</span>;
    - <span style="color:#83a598">**Legalidade</span>;**
    - **<span style="color:#83a598">Fé de ofício</span>** aos documentos;
- **Consequências**:
    - <span style="color:#83a598">**Produção de efeitos**</span> enquanto não decretada a invalidade do ato;
    - <span style="color:#83a598">**Autoexecutoriedade**</span>;
    - <span style="color:#83a598">**Inversão do ônus da prova**</span>;
    - <span style="color:#83a598">**Presunção relativa**</span>;
    - <span style="color:#83a598">**Controle judicial**.</span>

## **4.2 Imperatividade:**

- **Imposição** de **<span style="color:#d3869b">obrigações a terceiros</span>;**
- **<span style="color:#d3869b">Poder extroverso</span>;**
- **Depende** de **<span style="color:#d3869b">previsão legal</span>;**
- **Fundamento:**
    -  **<span style="color:#d3869b">Supremacia do interesse público</span>;**
- **Atos que impõem <span style="color:#d3869b">obrigações/restrições</span>;**
- **<span style="color:#fb4934">Ausentes</span>:**
    - **<span style="color:#d3869b">Atos negociais</span>;**
    - **<span style="color:#d3869b">Atos enunciativos</span>.**

## **4.3 Autoexecutoriedade:**

- **<span style="color:#fb4934">Execução imediata e direta</span>** pela administração, **<span style="color:#fb4934">sem necessidade de ordem judicial</span>;**
- Uso da força permitido;
- **<span style="color:#fb4934">Controle judicial</span>**;
- Poderes:
    - **<span style="color:#fb4934">de Polícia</span>;**
    - **<span style="color:#fb4934">disciplinar</span>;**
- **Fundamentos**:
    - **<span style="color:#fb4934">Presunção de legitimidade</span>**;
    - **<span style="color:#fb4934">Supremacia do interesse público</span>**;
    - **<span style="color:#fb4934">Urgência</span>.**
- **<span style="color:#fb4934">Exigibilidade</span>:**
    -  Meios **<span style="color:#fb4934">indiretos</span>** de coação;
    - O **<span style="color:#fb4934">administrado</span>** executa a medida.
- **<span style="color:#fb4934">Executoriedade</span>:**
    - Coação **<span style="color:#fb4934">direta</span>** ou **<span style="color:#fb4934">material</span>;**
    - **<span style="color:#fb4934">Uso da força</span>;**
    - A **<span style="color:#fb4934">administração</span>** executa a medida.

## **4.4 Tipicidade:**

- Previsão **<span style="color:#fb4934">legal</span>**;
- **<span style="color:#fb4934">Finalidade</span>** do ato;
- **<span style="color:#fe8019">Não existe</span>** ato **<span style="color:#fb4934">totalmente discricionário</span>**;
- **<span style="color:#fe8019">Não existe</span>** ato inominado **<span style="color:#fb4934">unilateral</span>**

# **5. Elementos de formação:**

- Requisitos ou aspectos de **<span style="color:#d3869b">validade</span>**;
- **Elementos <span style="color:#d3869b">ESSENCIAIS</span>**: _**(<span style="color:#fe8019">BIZU! Com Fi For M Ob</span>)**_  
    - **Competência**;
    - **Finalidade;**
    - **Forma;**
    - **Motivo;**
    - **Objeto.** 
- **Elementos <span style="color:#83a598">ACESSÓRIOS/ACIDENTAIS</span>**:
    - **<span style="color:#83a598">Ampliam</span>** ou **<span style="color:#83a598">restringem</span>** os efeitos jurídicos do ato;
    - Referem-se ao **<span style="color:#fe8019">OBJETO</span>** do **<span style="color: #d79921">ato discricionário</span>**;
    - **Termo**;
    - **Condição**:
    - **Modo** ou **encargo**.

| Elemento        | Tipo de Ato    | Característica                                                                                                                                                  | Sanável ou Insanável                                                                                                                                                                                           |
| --------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Competência** | Vinculado      | Definido pela lei; indica a atribuição legal do agente público para praticar o ato.                                                                             | Em regra, **insanável** se for competência exclusiva ou usurpação de função. Porém, admite-se algumas exceções, sendo possível ser **sanável** (em casos de competência relativa, como delegação ou avocação). |
| **Finalidade**  | Vinculado      | Deve sempre buscar o interesse público, sendo a finalidade geral e específica do ato.                                                                           | **Insanável** (desvio de finalidade torna o ato nulo, pois afeta o interesse público).                                                                                                                         |
| **Forma**       | Vinculado      | Modo de exteriorização do ato, geralmente previsto em lei para garantir clareza e transparência.                                                                | Em regra, **insanável** se for uma forma essencial prevista por lei. Todavia, pode ser **sanável** (se não for essencial à validade do ato);                                                                   |
| **Motivo**      | Discricionário | Justificativa fática e jurídica para a prática do ato. Em atos vinculados, é pré-determinado; em discricionários, pode haver avaliação da situação pelo agente. | **insanável** (a ausência ou falsidade do motivo compromete a validade do ato).                                                                                                                                |
| **Objeto**      | Discricionário | Conteúdo do ato, ou seja, o efeito jurídico que ele pretende produzir. Em atos vinculados, é determinado pela lei; em discricionários, há margem de escolha.    | Insanável (se o objeto for ilícito ou impossível).                                                                                                                                                             |

## **5.1 Competência (sujeito)**:

- **<span style="color:#83a598">Poder legal</span>** conferido ao agente para o **desempenho de suas atribuições**;
- Elemento **<span style="color:#83a598">vinculado</span>;**
- Exercício **<span style="color:#83a598">obrigatório</span>;**
- **<span style="color:#83a598">Irrenunciáveis;</span>**
- **<span style="color:#83a598">Intransferíveis/inderrogáveis</span>;**
- **<span style="color:#83a598">Imodificáveis</span>;**
- **<span style="color:#83a598">Imprescritíveis</span>;**
- **<span style="color:#83a598">Improrrogável</span>;**
- **<span style="color:#83a598">Critérios de distribuição</span>:**
    - matéria;
    - território;
    - grau hierárquico;
    - tempo;
    - fracionamento.
- **<span style="color:#83a598">Delegação</span>:**
    - quando **<span style="color:#fb4934">não houver impedimento legal</span>**, para órgãos ou agentes, **subordinados ou não;**
    - presente a **<span style="color:#83a598">hierarquia</span>:**
        - ato **<span style="color:#83a598">unilateral</span>**; 
        - ordem superior **independente da anuência** do delegado;
        - **<span style="color:#83a598">vertical</span>.**
    - **<span style="color:#fb4934">ausência</span>** de **hierarquia**:
        - **depende** da **concordância** do delegado;
        - ato **<span style="color:#fb4934">bilateral</span>;**
        - **<span style="color:#fb4934">horizontal</span>.**
- **<span style="color:#83a598">Avocação</span>:**
    - atrair para si **competência do subordinado**;
    - existência de **hierarquia;**
    - **excepcional;**
    - **motivos relevantes;**
    - **temporária;**
    - **<span style="color:#fb4934">vedada</span>** para **<span style="color:#fb4934">competência exclusiva</span>** do subordinado.

**_Se liga!_**  **Não podem ser objeto de delegação: (_Bizu! 🥕 CE  NO_** u **_RA_)**

- Edição de **atos de caráter NOrmativo;**
- Decisão de **Recursos Administrativos;**
- Matérias de **Competência Exclusiva** do órgão ou autoridade**.**

## **5.2 Finalidade**: 

- **<span style="color:#d3869b">Geral</span>**: **<span style="color:#83a598">interesse público</span>**;
- **<span style="color:#d3869b">Específica</span>**: objetivo diretamente **<span style="color:#83a598">previsto na lei</span>;**
- Elemento **<span style="color:#83a598">vinculado</span>.**

## **5.3 Forma**:

- Sentido **<span style="color:#d3869b">estrito</span>**:
    - modo de **<span style="color:#83a598">exteriorização</span>** do ato.
- Sentido **<span style="color:#d3869b">amplo</span>**:
    - **<span style="color:#83a598">formalidades</span>** do processo de formação da vontade;
    - princípio do **<span style="color:#83a598">devido processo legal</span>.**
- Elemento **<span style="color:#83a598">vinculado</span>;**
- **<span style="color:#83a598">Princípio da Solenidade:</span>**
    - forma **<span style="color:#83a598">específica</span>** prevista na **<span style="color:#83a598">lei</span>.**

## **5.4 Motivo:**

- **<span style="color:#d3869b">Situação</span>** de **<span style="color:#d3869b">direito</span>** ou de <span style="color:#d3869b">**fato</span>** que **<span style="color:#83a598">determina</span>** ou **<span style="color:#83a598">autoriza</span>** a realização do **<span style="color:#83a598">ato</span>** administrativo;
- **<span style="color:#d3869b">Situação</span>** de **<span style="color:#d3869b">direito</span>:** 
    - **<span style="color:#d3869b">requisitos</span>** previstos em **<span style="color:#d3869b">lei</span>.**
    - **<span style="color:#d3869b">Situação</span>** de **<span style="color:#d3869b">fato</span>:**
        - **<span style="color:#d3869b">concretização</span>** do pressuposto de **<span style="color:#d3869b">direito*</span>* no "mundo real".

**_Se liga!_**   Segundo **Celso Antônio Bandeira de Mello:**

- **<span style="color:#d3869b">Motivo</span>:** situação **objetiva, real, empírica, externa ao agente;  
    **
- **<span style="color:#d3869b">Motivação</span>:  intenção, vontade, propósito do agente** que editou o ato;
    - subjetiva, interna e psicológica do agente.

**_Não confunda!_**

- **<span style="color:#d3869b">Motivação*</span>* é a **demonstração dos motivos (**justificativa do ato**)! Portanto, nem todos os atos precisam ter motivação:
    -  **<span style="color:#fb4934">exoneração*</span>* de ocupante de **<span style="color:#fb4934">cargo de provimento em comissão</span>.**
- **<span style="color:#d3869b">Motivo*</span>* é **elemento de formação** do ato, logo, presente em todos os atos!

- Ausência de motivação em um ato administrativo corresponde a um vício no elemento **forma**

**_Importante saber!_** 

- Motivação **<span style="color:#d3869b">contextual</span>**: a própria autoridade enumera os motivos de forma expressa;
- Motivação **<span style="color:#d3869b">aliunde</span>**, ou **<span style="color:#d3869b">por referência</span> _(per relationem)_**:  referência a um documento anterior, utilizando-o como fonte de suas razões para decidir.
- **<span style="color:#d3869b">Teoria dos motivos determinantes</span>:** a **<span style="color:#d3869b">validade do ato se vincula aos motivos*</span>* indicados como seu fundamento:
    - Aplicado aos atos <span style="color:#fb4934">**discricionários*</span>* ou **<span style="color:#fb4934">vinculados</span>**;
    - **<span style="color:#d3869b">Motivação</span> <span style="color:#fb4934">obrigatória ou não**;</span>
    - **<span style="color:#d3869b">Motivos</span>** indicados **<span style="color:#d3869b">inexistentes</span>** ou **<span style="color:#d3869b">falsos</span>**: **<span style="color:#fb4934">ato NULO.</span>**

## **5.5 Objeto (conteúdo)**:

- **<span style="color:#83a598">Efeito</span>** jurídico do ato;
- **<span style="color:#83a598">Vinculado**</span> ou **<span style="color:#83a598">discricionário</span>**;
- **<span style="color:#83a598">Lícito, possível, certo e moral.</span>**

**_Não confunda!_** 

- **<span style="color:#fe8019">Objeto</span>**: **<span style="color:#83a598">fim imediato</span>** (direto) do ato;
- **<span style="color:#fe8019">Finalidade</span>**:  **<span style="color:#83a598">resultado desejado</span>**, **<span style="color:#fe8019">fim mediato</span>** (indireto).

💡**_Se liga!_**   Segundo **Maria S. Z. Di Pietro:**

- **<span style="color:#fe8019">Objeto natural</span>:** efeito jurídico que o ato produz, decorrente da própria natureza do ato, previsto em lei;
- **<span style="color:#fe8019">Objeto acidental/acessório</span>:** efeito jurídico que o ato produz em decorrência de cláusulas acessórias inseridas, tais como:
    - **<span style="color:#d3869b">termo</span>:** data de início ou término da eficácia do ato;
        - evento **<span style="color:#d3869b">FUTURO</span>** e **<span style="color:#d3869b">CERTO</span>.**
    - **<span style="color:#fb4934">condição</span>:**
        - evento **<span style="color:#fb4934">FUTURO</span>** e **<span style="color:#fb4934">INCERTO</span>;**
        - **<span style="color:#fb4934">suspensiva</span>:**  **suspende** o início da eficácia do ato **<span style="color:#fb4934">até o cumprimento da condição</span>**;
        - **<span style="color:#fb4934">resolutiva</span>:** **cessa** a produção dos **efeitos jurídicos** do ato **<span style="color:#fb4934">após o** **adimplemento da condição.</span>**
    - **<span style="color:#83a598">encargo/modo</span>:** obrigação ou ônus para que o destinatário usufrua do benefício do ato;
    - **somente pode ocorrer em atos discricionários.**

# **6. Vícios dos atos administrativos:**

## **6.1 De competência:**  
 - **Incompetência**:
     - **<span style="color:#d3869b">Excesso de poder</span>:** 
	     - Passível de **convalidação (em regra);**
	     - **<span style="color:#fb4934">Vício insanável</span>: competência EXCLUSIVA;**
    - **<span style="color:#d3869b">Função de fato</span>:** 
	    - Exercida pelo “agente de fato” (sua investidura não ocorreu de forma normal ou regular);
	    - Teoria da aparência de legalidade: ato praticado em função de fato é considerado válido (boa-fé)
    - **<span style="color:#d3869b">Usurpação de poder</span>:**
	    - Uma pessoa pratica ato administrativo sem ter qualquer forma de investidura na função pública;
	    - Crime previsto no art. 328 do Código Penal;
	    - Ato inexistente.
	- **Incapacidade**: 
		- Passíveis de convalidação por autoridade que não esteja na mesma situação de impedimento e de suspeição;
	- **<span style="color:#83a598">Impedimento</span>**:
		- hipóteses objetivas;
		- interesse direto na matéria;
		- presunção absoluta de incapacidade;
		- deve ser declarada pela autoridade;
	    - nulidade desde a origem.
	- **<span style="color:#83a598">Suspeição</span>**:
		- hipóteses subjetivas;
		- amizade íntima ou a inimizade notória dos interessados;
		- presunção de incapacidade é relativa;
		- considerados válidos, sanados antes da arguição.  
                              
                            
## **6.2 De finalidade:**
- **Desvio de poder ou de finalidade:**
	- Finalidade distinta do interesse público;
	- Finalidade específica diferente da prevista em lei;
	- Vício **<span style="color:#fb4934">insanável</span>**;
	- Ato **<span style="color:#fb4934">nulo</span>**.  
                  
                
## **6.3 De forma:**
- **<span style="color:#fb4934">Forma prevista em lei não</span>** foi **<span style="color:#fb4934">observada</span>**;
- Formalidade ou procedimento para a tomada de decisão **<span style="color:#fb4934">não seguiu o rito definido em lei</span>**;
	- **<span style="color:#83a598">Sanável</span>**, em alguns casos, **<span style="color:#83a598">por convalidação</span>**:
		- Exemplo: o interessado em um processo administrativo não é notificado nos termos definidos em lei, mas comparece voluntariamente na data marcada.
	- **<span style="color:#fe8019">Insanável</span>** quando afetar o ato em seu próprio conteúdo (**<span style="color:#fe8019">forma</span>** é **<span style="color:#fe8019">essencial</span>** ao ato).

_**Se liga!**_   A **<span style="color:#fb4934">motivação compõe a forma</span>** do ato! Sua **<span style="color:#fb4934">ausência</span>** quando for obrigatória acarretará a **<span style="color:#fb4934">nulidade</span>** do ato **<span style="color:#fb4934">por vício de forma</span>.**

- **<span style="color:#83a598">De motivo</span>:**
        <span style="color:#83a598">- **Falso**;
        - **Inexistente**;
        -</span> **<span style="color:#83a598">Juridicamente inadequado*</span>*/**ilegítimo**;
        - **<span style="color:#fb4934">Insanável</span>**.

_**Bizu!**_  **ConForma convalida**

<span style="color:#b8bb26">-  **De objeto:**
    - **Proibido por lei** ou com conteúdo **não previsto em lei**;
    - **Diverso do previsto na lei** para a situação;
    - **Impossível**;
    - **Imoral**;
    - **Incerto</span>** em relação aos destinatários, às coisas, ao tempo, ao lugar.   
          
        
# **7. Mérito do ato administrativo:**
- **<span style="color:#d3869b">Atos vinculados</span>:**
	- **<span style="color:#fb4934">Sem margem</span>** de liberdade de **decisão**;
	- **Lei define todos os <span style="color:#d3869b">elementos de formação</span>** do ato administrativo;
	- **<span style="color:#fb4934">Não existe mérito</span>** ou juízo de **<span style="color:#fb4934">conveniência e oportunidade</span>**.

- **<span style="color:#83a598">Atos discricionários</span>:**
	- **<span style="color:#83a598">Margem de liberdade</span>** para o agente público quanto ao:  
		- **<span style="color:#83a598">Motivo</span>**:  
			- **<span style="color:#fb4934">lei não define</span>** os **exatos motivos** de edição do ato;
			- lei emprega **conceitos jurídicos indeterminados** (não possuem exatidão quanto ao seu sentido).
		- **<span style="color:#b8bb26">Objeto</span>**:  
			- lei definir **mais do que um conteúdo legalmente válido**:
			- **distintas soluções;**
			- **limites máximos e mínimos** para o ato.
			- **<span style="color:#fe8019">Conveniência e oportunidade</span>.**

**_Se liga!_**   Há **entendimento doutrinário** que:

- **<span style="color:#fb4934">Forma</span>** também poderá ser **discricionária** quando a <span style="color:#fb4934">**lei:</span>**  
    - **<span style="color:#fb4934">Não definir a forma</span>** a ser adotada;
    - Mencionar **<span style="color:#fb4934">mais do que uma forma admitida</span>**.
- **<span style="color:#83a598">Finalidade</span>** geral (interesse público tem certa discricionaridade):  
    - A própria definição “interesse público” depende de **<span style="color:#83a598">interpretação</span>** e avaliação da autoridade pública.

**Mas as provas costumam adotar a cobrança tradicional em que somente o motivo e o objeto são considerados discricionários.**

  
**Controle de mérito:**

**<span style="color:#83a598">[[13. Poder Judiciário]]</span>:**

- Realiza o **controle de legalidade** e, identificando o ato ilegal, poderá anulá-lo;
- Não realiza controle de mérito, de tal forma que não poderá, por exemplo, revogar um ato administrativo.

**<span style="color:#fe8019">Controle da razoabilidade e da proporcionalidade*</span>*:

- **Limitam a discricionariedade** administrativa;
- **<span style="color:#fb4934">Não há controle de mérito*</span>*;
- **Abuso** da autoridade (**ilegalidade**);
- Juízo quanto à **juridicidade ou legitimidade** do ato administrativo;
- **Judiciário** ou a própria **administração**.

# **8. Classificação:**

## **8.1 📌 Quanto aos destinatários:**

- <span style="color:#83a598">**Gerais/normativos</span>:**   
    - **<span style="color:#fb4934">Não possuem</span>** **<span style="color:#fb4934">destinatários determinados</span>**;
    - **Generalidade e abstração**.
- **<span style="color:#83a598">Individuais</span>:**  
    - **Destinatários determináveis, certos**;
    - **Caso concreto**.

_**Se liga!**_   Segundo **Maria Di Pietro, ato normativo:** 

- **<span style="color:#fb4934">Não pode ser impugnado diretamente pela pessoa lesada na via judicial*</span>*  (competência somente das **pessoas previstas no art. 103 da [[CF]]** para **propor inconstitucionalidade de ato normativo**);
- **Precedência hierárquica** sobre o ato individual;
- **<span style="color:#fb4934">Não pode ser impugnado por meio de recursos administrativos*</span>*;
- **<span style="color:#b8bb26">Sempre revogável</span>**;
- **Ato individual** tem **<span style="color:#fb4934">limitações para revogação</span>** (exemplo:  **direitos subjetivos a favor do administrado**).

## **8.2 Quanto à sua estrutura:**

- **<span style="color:#d3869b">Concretos</span>**:
    - dispõem de um **caso único e específico;** 
    - **equivalem aos individuais.**
- **<span style="color:#fb4934">Abstratos</span>**:
    - disciplinam situações reiteradas, com **aplicações ilimitadas;**
    - **correspondem aos atos gerais.**

## **8.3 Quanto ao âmbito de aplicação:**

- **<span style="color:#83a598">Internos</span>:**  
    - efeitos no **interior da administração** pública (**órgãos e agentes**);
    - em regra, não geram direitos adquiridos;
    - revogáveis a qualquer tempo;
    - exemplo: portaria, ordem de serviço interna, etc.
- **<span style="color:#b8bb26">Externos:</span>**
    - tratam de direitos, obrigações, negócios ou conduta perante a Administração;
    - devem ser publicados;
    - possuem efeitos jurídicos externos à repartição;
    - atos que onerem o patrimônio público.

## **8.4 Quanto às prerrogativas de:**

- **<span style="color:#fe8019">Império</span>**: **coerção;**
- **<span style="color:#83a598">Gestão</span>**: **horizontalidade;**
- **<span style="color:#d3869b">Expediente</span>**: **rotina.**

## **8.5 Quanto à liberdade de ação:**

- Vinculados;
- Discricionários

## **8.6 Quanto à formação ou intervenção da vontade administrativa:**

- **<span style="color:#83a598">Simples</span>**: **<span style="color:#83a598">manifestação de vontade</span>** de um **único órgão;**
- **<span style="color:#b8bb26">Complexo</span>**:
    - **<span style="color:#b8bb26">manifestação de vontade</span>** de **dois ou mais órgãos;**
    - **<span style="color:#b8bb26">ATO ÚNICO</span>**
- **<span style="color:#d3869b">Composto</span>**:   
    - **<span style="color:#d3869b">manifestação de vontade</span>** de um **único órgão;**
    - **<span style="color:#d3869b">DOIS ATOS (principal e acessório</span>)**

**_Se liga!_**    **Jurisprudência:**

- Processos perante o TCU deve respeitar o **<span style="color:#83a598">direito ao contraditório e à ampla defesa</span>** quando da decisão **puder resultar anulação ou revogação** de ato administrativo que beneficie o interessado, **<span style="color:#fb4934">salvo a apreciação da legalidade do ato de _concessão inicial_ de aposentadoria, reforma e pensão.</span>**

_**⚠️Importante saber:**_

- Concessão de **aposentadoria, reforma e pensão** é **<span style="color:#83a598">ato complexo</span>**:
    - **<span style="color:#b8bb26">Manifestação</span>** do **órgão do servidor** e do **Tribunal de Contas**;
- **Tribunal de Contas**:  
    - **<span style="color:#fb4934">Não precisa conceder contraditório**</span> caso indefira o registro;
    - Tem o prazo de **<span style="color:#fe8019">cinco anos</span>** para apreciar a aposentadoria, sob pena de **deferimento tácito do registro**;
    - Prazo **<span style="color:#fb4934">decadencial</span> de <span style="color:#fe8019">cinco anos</span>**, **a partir do deferimento do registro**, para **<span style="color:#fb4934">anular</span>** a aposentadoria;
- **Após o registro**, o **ato está formado;**
- **<span style="color:#fb4934">Desfazimento</span> dependerá**:   
    - **<span style="color:#83a598">de contraditório*</span>*; 
    - **da** **<span style="color:#83a598">participação</span>** do **TCU** e do **órgão**.

# **9. Quanto aos efeitos:**

- <span style="color:#d3869b">**Constitutivo**</span>:
    - <u>Cria</u> uma **nova situação jurídica** (direito ou obrigação);
    - **Exemplos**: licenças, autorizações; sanções.
- **<span style="color:#83a598">Modificativo</span>**:
    - <u>Altera</u> uma **situação jurídica preexistente.**
- **<span style="color:#fe8019">Extintivo/desconstitutivo</span>:** 
    - <u>Encerra</u> uma **situação jurídica;**
    - **Exemplos:** cassação, caducidade.
- **<span style="color:#b8bb26">Declaratório</span>:**
    - <u>Reconhece</u> uma **situação preexistente;**
    - <u>Atesta</u> **fato/situação.**

**_💡Se liga!_**  

**Hely Lopes Meirelles:**

- **<span style="color:#b8bb26">Alienativos</span>**:  <u>Transferência</u> de **bens**.
- **<span style="color:#fb4934">Abdicativos</span>**: <u>Renúncia</u> de **direito** em caráter definitivo (irreversível, imodificável)

 **José dos Santos Carvalho Filho e Maria Di Pietro**:

- **<span style="color:#d3869b">Constitutivo</span>**:
    - Cria, modifica ou extingue **direito ou situação;**
- **<span style="color:#b8bb26">Declaratório</span>**: 
    - Reconhece um **direito prévio**;
- **<span style="color:#83a598">Enunciativo</span>**:
    - Indicam **juízo de valor**;
    - Dependem de outro ato decisório.

# **10. Quanto à eficácia:**

- **<span style="color:#83a598">Válido</span>**:
    - Observa **todos os requisitos legais**;
    - **Sem vícios.**
- **<span style="color:#fb4934">Nulo</span>**:
    - **Vício insanável;**
    - Anulado pela **Administração** ou pelo **Judiciário.**
- **<span style="color:#b8bb26">Anulável</span>**:
    - Passível de **convalidação;**
    - Anulado pela **Administração** ou pelo **Judiciário.**
- **<span style="color:#d3869b">Inexistente</span>:**
    - Apenas **aparência** de **manifestação de vontade** da administração;
    - **<span style="color:#fb4934">Sem aperfeiçoamento</span>** do ato;
    - **Usurpador** de função pública;
    - **<span style="color:#fb4934">Sem efeitos</span>**;
    - Ausência de **<span style="color:#fb4934">responsabilidade civil do Estado;</span>**
    - Ausência de **<span style="color:#fb4934">prescrição e decadência</span>.**
- **<span style="color:#d3869b">Irregulares</span>**:
    - **Vícios materiais** **irrelevantes**;
    - **<span style="color:#b8bb26">Anulável</span>.**

**_Se liga!_   Caso a questão seja sobre classificação:**

- **<span style="color:#d3869b">Atos “irregulares”</span> = “vício material irrelevante”  
    **

**Demais casos:**

- **Termo _"irregular_ "= "<span style="color:#fb4934">viciado, ilegal, inválido</span>"**

# **11. Quanto à exequibilidade:**

- **<span style="color:#fb4934">Perfeito</span>**:   
    - Ciclo de formação **completo**;
    - Ato **pronto**.
- **<span style="color:#d3869b">Imperfeito</span>**:
    - Ainda **em formação.**
- **<span style="color:#b8bb26">Eficaz</span>**:
    - Produz **efeitos**;
    - **Exequíveis**.
- **<span style="color:#83a598">Pendente</span>**:
    - **Condição suspensiva**;
    - **Termo.**
- **<span style="color:#fe8019">Consumado</span>**: 
    - **Exaurido**;
    - **Já produziu todos os efeitos.**

# **12. Espécies de atos administrativos:**

## **12.1 Atos normativos:**

- **Generalidade e abstração**;
- Impessoais;
- Exemplos: decretos, leis, regulamentos;
- **Discricionários;**
- **<span style="color:#fb4934">Não produzem efeitos jurídicos imediatos</span>;**
- Atos em **sentido formal;**
- **<span style="color:#fb4934">Não primários</span>** (**<span style="color:#fb4934">não inovam</span>** o ordenamento jurídico);
- **<span style="color:#fb4934">Impossibilidade de recurso administrativo</span>** contra ato normativo;
- Controle judicial;
- **Caráter vinculante** ao órgão ou entidade a que se destinam.

## **12.2 Atos ordinatórios:**

- **Disciplinar** o **funcionamento** da administração;
- **Disciplinar** a **conduta** dos agentes públicos;
- Alcance **interno**;
- Exemplos: instruções, ofícios, portarias.

## **12.3 Atos Enunciativos:**

- São os atos pelos quais a administração declara um fato pré-existente, profere uma opinião ou emite um juízo de valor, sem que, por si só, produza consequências jurídicas.

- **Declara** um **fato** pré-existente;
- **Expressa** uma **opinião** ;
- **Manifesta** um **juízo de valor**;
- **<span style="color:#fb4934">Não produzem efeitos jurídicos imediatos</span>;**
- **<span style="color:#fb4934">Não constituem manifestação de vontade</span>;**
- Atos em **sentido formal;**
- Exemplo: **c**ertidão, **a**postilas, **p**areceres, **a**testados (**capa**).

## 12.5 Atos negociais/de consentimento:

- **Vontade** da administração coincidente com a pretensão de um particular;
- Anuência/consentimento **prévio**;
- Ato **unilateral**;
- Dependem de **solicitação**;
- **Efeitos individuais, específicos e concretos**;
- Ausência **<span style="color:#fb4934">de imperatividade</span>**
- **<span style="color:#fb4934">Não há autoexecutoriedade e coercibilidade</span>**;
- **Vinculados** (exemplo: **licença**) ou **discricionários** (**autorizações, permissões**);
- **Definitivos** ou **precários;**
- **Exemplos:** Permissão, homologação, visto, dispensa, admissão, renúncia.

**_🏴_** **_Bizu!_** 
- **_Se tem R é discricionário: Autorização, permissão, aprovação, renúncia_**

- **_Se não tem R é vinculado: Licença, homologação, admissão_**

- **_<span style="color:#fe8019">Exceção: Dispensa é discricionário (dispensa o R)</span>_**

## 12.6 Atos punitivos ou sancionatórios:

- **<span style="color:#83a598">Internas</span>**:

    - Regime funcional ([[5. Poderes e Deveres|poder disciplinar]]);
    - Exemplos: Advertência, suspensão, demissão.

- **<span style="color:#fb4934">Externas</span>**:

    - Exemplos: Multas ([[5. Poderes e Deveres|poder de polícia]]), apreensão de bens, destruição de coisas.

# **13. Extinção dos atos administrativos:**

## **13.1 Modalidades de desfazimento**:

- Um ato administrativo se extingue por:
    - **<span style="color:#83a598">extinção natural</span>**:
        - produziu todos os efeitos;
        - **<span style="color:#83a598">esgotamento do conteúdo jurídico</span>;**
        - **<span style="color:#83a598">execução material</span>;**
        - cumprimento da **<span style="color:#83a598">condição resolutiva ou termo final;</span>**
    - **<span style="color:#d3869b">extinção subjetiva:</span>**
        - **<span style="color:#d3869b">desaparecimento do sujeito beneficiário</span>** do ato;
        - **<span style="color:#d3869b">ato intransferível</span>** a terceiros (ato **<span style="color:#d3869b">personalíssimo – _intuitu personae_</span>**).
    - **<span style="color:#fb4934">extinção objetiva:</span>**
        - objeto **<span style="color:#fb4934">desaparece</span>.**
    - **<span style="color:#b8bb26">renúncia</span>:**
        - **<span style="color:#b8bb26">beneficiário</span>** do ato **<span style="color:#b8bb26">abdica</span>** de uma vantagem que desfrutava.
    - **<span style="color:#83a598">retirada</span>**: 
        - **<span style="color:#83a598">caducidade</span>**:
            - **legislação superveniente contrária** à legislação que fundamentou o ato anterior.
        - **<span style="color:#83a598">contraposição/derrubada</span>**:
            - **novo ato** administrativo se **contrapõe** ao ato **anterior**.
        - **<span style="color:#83a598">cassação</span>**: 
            - **desfazimento** de um **ato válido** por **descumprimento das condições**;
            - ato **vinculado**;
            - ato **sancionatório**.
        - **<span style="color:#83a598">anulação</span>**:
            - desfazimento de ato **ilegal**;
        - **<span style="color:#83a598">revogação</span>**:
            
            - desfazimento de um **ato válido**, **inconveniente e inoportuno**.
                

## **13.2 Autotutela**:

- **<span style="color:#fb4934">capacidade</span>** da administração de **<span style="color:#fb4934">controlar os seus próprios atos</span>;**
- controle de **<span style="color:#fb4934">mérito</span>** ou de **<span style="color:#fb4934">legalidade</span>**;
- por **<span style="color:#fb4934">provocação</span>**;
- **<span style="color:#fb4934">de ofício</span>**;
- **<span style="color:#fb4934">devido processo legal</span>**;
- **<span style="color:#fb4934">contraditório</span>** e da **<span style="color:#fb4934">ampla defesa</span>**.

📌 **Controle judicial:**

- **<span style="color:#fb4934">não pode</span>**:
    - invadir o **<span style="color:#fb4934">mérito</span>**;
    - ser de **<span style="color:#fb4934">ofício</span>.**

💡**<span style="color:#fb4934">_Se liga</span>!_**  **Súmula 473- STF:**

- A **administração** pode **anular** seus **próprios atos**, quando eivados de **vícios** que os tornam **ilegais**, porque deles **<span style="color:#fb4934">não se originam direitos</span>**; ou **revogá-los, por motivo de conveniência ou oportunidade, <span style="color:#b8bb26">_respeitados os direitos adquiridos_</span>**, e _**<span style="color:#b8bb26">ressalvada</span>**_, em todos os casos, a **<span style="color:#b8bb26">_apreciação judicial_</span>**.

## 13.3 **Revogação:** 

- recai **<span style="color:#d3869b">sobre ato</span>**:
    - **<span style="color:#d3869b">válido</span>**: sem vícios;
    - **<span style="color:#d3869b">eficaz</span>**: produzindo efeitos;
    - **<span style="color:#d3869b">discricionário</span>**.
- pela **<span style="color:#d3869b">mesma autoridade</span>** que editou o ato **<span style="color:#d3869b">ou superior</span>**;
- efeitos **<span style="color:#d3869b">_ex nunc_</span>**;
- **<span style="color:#d3869b">ab-rogação</span>**: **total** (_**<span style="color:#fe8019">BIZU</span>!**_ **<span style="color:#d3869b">Ab</span>**-rogação = revogação **<span style="color:#d3869b">Absoluta</span>**);
- **<span style="color:#d3869b">derrogação</span>**: **parcial**
- **<span style="color:#fb4934">não é possível revogar ato</span>**:
    - **<span style="color:#fb4934">exaurido</span>**;
    - **<span style="color:#fb4934">ilegais, inválidos, viciados</span>**;
    - **<span style="color:#fb4934">vinculados</span>**;
    - com **<span style="color:#fb4934">competência exaurida</span>** relativamente ao objeto do ato;
    - **<span style="color:#fb4934">meramente administrativo</span>**;
    - **<span style="color:#fb4934">integrante de procedimento</span>**;
    - que **<span style="color:#fb4934">gerem direito adquirido</span>**.

- **️Convalidar um ato** <u>significa _“corrigir”_ um vício sanável</u>.
    - **Vícios sanáveis (<span style="color:#83a598">FOCO</span>):**
        - **<span style="color:#83a598">FO</span>rma**, <u>desde que não essencial à validade do ato</u>.
        - **<span style="color:#83a598">CO</span>mpetência**, <u>desde que não exclusiva</u>.
- **A ratificação e a confirmação** <u>são consideradas espécies de convalidação</u>.

**Conversão x Convalidação:**

- **Conversão** = Transformação de um ato em **OUTRO.**
- **Convalidação** = Correção feita no ato, Continua o **MESMO** ato.

## **13.4 CONVALIDAÇÃO**

1. Somente são passíveis de **<u>convalidação</u>** os vícios de **<span style="color: #d79921">Forma</span>** e **<span style="color: #d79921">Competência</span>** (**FO**-**CO**)

⚠️ Se o vício for de competência **<span style="color: #d79921">EXCLUSIVA</span>, <span style="color:#fb4934">NÃO É SANÁVEL</span>**.

⚠️ **ATENÇÃO!!!** Carvalho Filho traz a possibilidade de **convalidação do objeto se o ato for PLÚRIMO** (um ato com mais de um objeto). Assim, seria possível aproveitar o ato em parte.

1. Requisitos:
	- **<span style="color:#fb4934">não</span>** pode prejudicar terceiros;
	- deve visar a realização do **interesse público**;
	- deve recair sobre **vícios sanáveis;**
	- o assunto **<span style="color:#fb4934">não</span>** ter sido objeto de **impugnação administrativa ou judicial** pelo interessado, **<span style="color: #d79921">exceto</span>** se se tratar de **<u>irrelevante formalidade</u>**, pois, neste caso, os atos são sempre convalidáveis[.](https://www.tecconcursos.com.br/questoes/455505)  
		
	- **<u>CONTROLE JUDICIAL</u>**
            
		1. Numa eventual ação judicial para suprir o **silêncio da Administração,** o <u>[[13. Poder Judiciário]] pode decidir</u> de formas diferentes, de acordo com o tipo de ato não proferido pela Administração:
			- Caso se trate de ato **<span style="color:#83a598">DISCRICIONÁRIO</span>**, o **JUDICIÁRIO** irá DETERMINAR que a **<span style="color:#83a598">ADMINISTRAÇÃO se MANIFESTE</span>.**
			- Por outro lado, caso se trate de ato **<span style="color:#b8bb26">VINCULADO</span>**, o próprio **<span style="color:#b8bb26">JUDICIÁRIO PODERÁ SUPRIR a OMISSÃO e EMITIR o ATO no lugar da Administração</span>**, se verificar que estão presentes todos os requisitos previstos em lei para a prática do ato.